...
parser.add_argument('--zip', action='store_true',
                    help='use zipped data instead of folder data')
parser.add_argument('--cache-mode', type=str, default='part', choices=['no', 'full', 'part'],
                    help='no: no cache, '
                    'full: cache all data, '
                    'part: sharding the data into nonoverlapping pieces and only cache one piece')
...
def net_factory(net_type="unet", in_chns=1, class_num=3, n_sdf_channels=3, n_kp_channels=4,
                 l_rate=0.01):
    if net_type == "unet":
        ...
        net = SAMHyDAS_Net(nin=in_chns, n_classes=class_num, n_sdf_channels=n_sdf_channels, n_kp_channels=n_kp_channels, l_rate=l_rate).cuda()
